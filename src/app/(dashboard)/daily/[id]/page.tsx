...
export default function DailyReportDetailPage() {
  ...
  const fetchReport = async () => {
    try {
      const response = await fetchWithAuth(`/api/daily-reports/${id}`);
  ...
  const handleUpdate = async () => {
    ...
    try {
      ...
      const response = await fetchWithAuth(`/api/daily-reports/${report.id}`, {
        method: 'PUT',
        body: JSON.stringify({
          date: dateStr,
          title,
  ...
  return (
    <div>
      ...
      {/* ── Hero: date + mood ── */}
      <section className="relative bg-primary overflow-hidden rounded-xl mb-8">
        {/* Grain texture */}
      ...
      <div ref={contentRef}>
        ...
        <div className="border-t border-border/30 py-6 flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-6 text-xs text-muted-foreground/60">
          <span className="inline-flex items-center gap-1.5">
            ...
            创建于 {formatTime(report.createdAt)}
          </span>
          {report.updatedAt && report.updatedAt !== report.createdAt && (
            <span className="inline-flex items-center gap-1.5">
              <ClockIcon className="w-3 h-3" />
              更新于 {formatTime(report.updatedAt)}
            </span>
          )}
        </div>
      </div>
