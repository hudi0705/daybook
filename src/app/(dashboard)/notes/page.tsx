...
import { CategoryPicker } from '@/components/features/notes/category-picker';
import {
  BookOpenIcon,
  SearchIcon,
  PlusIcon,
...
export default function NotesPage() {
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      const response = await fetchWithAuth(`/api/notes?id=${id}`, { method: 'DELETE' });
      const result = await response.json();
      if (result.success) {
          ...
          <section className="flex-1 min-w-0">
            ...
            {loading ? (
              ...
              <div className="py-24 flex flex-col items-center justify-center text-center">
                ...
                {!searchQuery && !selectedTag && (
                  <Button size="sm" className="gap-1.5 text-xs" onClick={() => navigate('/notes/new')}>
                    <PlusIcon className="w-3.5 h-3.5" />
                    新建笔记
                  </Button>
                )}
