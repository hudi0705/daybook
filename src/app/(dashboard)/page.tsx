...
import { ContributionGraph } from '@/components/features/daily/contribution-graph';
import {
  DropdownMenu,
  DropdownMenuContent,
  DropdownMenuItem,
...
export default function HomePage() {
  ...
  const fetchContribution = async () => {
    try {
      const response = await fetchWithAuth('/api/contribution');
      const result = await response.json();
      if (result.success) {
            ...
            {contributionLoading ? (
              ...
              <ContributionGraph
                ...
                onDayClick={(date) => {
                  ...
                  if (report) navigate(`/daily/${report.id}`);
                }}
              />
            )}
          </div>
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 pt-4 pl-[34px]">
                          ...
                          {weekReports.map((report) => (
                            <Card
                              ...
                              onClick={() => navigate(`/daily/${report.id}`)}
                            >
                              <CardContent className="p-4">
